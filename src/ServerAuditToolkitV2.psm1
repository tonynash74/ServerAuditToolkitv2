...
function New-SATReport {
  ...
  function _TableFromRows($title,$headers,$rows,$maxRows){
    $html = "<h3 id='" + (_HtmlEnc ($title -replace '\s','-')).ToLower() + "'>" + (_HtmlEnc $title) + "</h3><div class='table-responsive'>"
  ...
  if ($Data['Get-SATExchange']) {
    foreach ($srv in @($Data['Get-SATExchange'].Keys)) {
      ...
      if ($d) {
        $exMeta += New-Object PSObject -Property @{Server=$srv;Version=$d.Version;Edition=$d.Edition;Roles=((@($d.Roles) -join ', '))}
      }
  ...
  # SBS add-ons (if present): WSUS, SharePoint, RRAS, Fax, POP3, RWA, LOB signatures
  $wsusRow=@();$wsusSvc=@()
  if ($Data['Get-SATWSUS']) {
  ...
  if ($Data['Get-SATRWA']) {
    foreach ($srv in @($Data['Get-SATRWA'].Keys)) {
      ...
      if ($d -and $d.CertIssues){
        foreach ($i in $d.CertIssues){ $rwaIssues += New-Object PSObject -Property @{Server=$srv;Issue=$i} }
      }
    }
  }
  ...
  # Certificates expiring soon (rule match id if present)
  $expSoon = 0
  if ($Findings){
    foreach ($f in $Findings){
