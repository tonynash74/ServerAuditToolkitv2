...
try {
    # Determine the appropriate user Modules folder. Use the user's Documents folder
    # (works when Documents is redirected to OneDrive) and fall back to $env:USERPROFILE path.
    $documents = [Environment]::GetFolderPath('MyDocuments')
    $userModules = Join-Path $documents 'PowerShell\Modules'
    if (-not (Test-Path $userModules)) {
        # If Documents path doesn't resolve, try the legacy location
    ...
    if (-not (Test-Path $manifestCandidate)) {
        # fallback: look in repo root and src
    ...
    # Copy the entire src folder (preserve relative paths expected by the manifest)
